...
\begin{definition}[\(k\)-continuously differentiable map]
  A map \(f: G \to \R\) is said to be \(k\)-continuously differentiable if for
  all tuples of indices \(I_i = (j_1, \dots, j_i)\) --- for all \(1 \leq i \leq
...
\begin{proof}
  ...
  where the map \(\ell \mapsto \left\langle \ell, \Hess(f(x_0)) \ell \right\rangle\) assumes its minimum
  \(m < 0\) and \(v \in S^{n-1}\) be the point where the map assumes it maximum
  \(M > 0\). Define \(t > 0\) so that \(x_0 + ut \in U\), then
  \[
