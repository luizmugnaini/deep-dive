...
\(f: \Omega_X \times \Omega_Y \to \R\). The \emph{marginalisation property}
states that the marginal probability of \(X = x\) is given by
\[
f(x) =
\begin{cases}
  \sum_{y \in \mathcal{Y}} f(x, y), &\text{if } y \text{ is discrete} \\
  \int_{\mathcal{Y}} f(x, y)\, \diff y, &\text{if } y \text{ is continuous}
\end{cases}
\]
\end{lemma}
...
Given a joint probability \(f: \Omega_X \times \Omega_Y \to \R\), the product
rule states that
\[
f(x, y) = f(y \mid x) f(x).
\]
...
\(f(x, y) = f(x \mid y) f(y)\), resulting in
\[
...
\emph{prior} knowledge \(f(x)\) and a second random variable \(Y\) of which we
can observe and have a \emph{marginal evidence} \(f(y)\) and a relational
\emph{likelihood} \(f(y \mid x)\). This data gives a \emph{posterior} knowledge
\(f(x \mid y)\).

\begin{remark}
\label{rem:likelihood}
The value \(f(y \mid x)\) can be called either the ``likelihood of \(x\) given
...
= \Cov(X, Y)^{\Transp} \in \R^n \times \R^m,
\]
