...
continuity of \(\pi\) by defining a topology \(\mathcal T\) on \(S\) so that
...
quotient topology \(\mathcal T\) on the set \(S\) is the intersection
\(\mathcal T = \bigcap_{T \in T_S}T\). Even better than that definition is the
...
\(f: (S, \mathcal T) \to Z\) such that the diagram
\[
  \begin{tikzcd}
...
\(\Id'^{-1}(U) = U \subseteq (S, \mathcal T')\) is open, hence
\(\mathcal T \subseteq \mathcal T'\). Analogously, let \(Z = (S, \mathcal T')\)
and consider \(f = \Id: (S, \mathcal T) \to (S, \mathcal T')\). Let
\(U' \in \mathcal T'\) then from the continuity of \(\Id\) we find that
\(\Id^{-1}(U') = U' \subseteq (S, \mathcal T)\) is open, therefore
\(\mathcal T' \subseteq \mathcal T\). Thus indeed \(\mathcal T = \mathcal T'\)
as wanted.
\end{proof}
...
exists a \emph{unique} morphism \(f_{*}: Y \unique Z\) sucht that the following
diagram commutes
\[
\begin{tikzcd}
X \ar[d, two heads, "q", swap] \ar[dr, "f"] & \\
...
hence \(x' \in U'\) --- wich implies that \(U'\) is open.
\end{proof}
