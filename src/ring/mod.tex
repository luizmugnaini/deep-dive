...
\begin{proof}
% Define a map \(q: M \to \ker p\) as \(q|_{\ker p} = \Id_{\ker p}\) and
% \(q|_{M \setminus \ker p} = 0\). We shall prove that this is an \(R\)-linear
% map:
% \begin{itemize}\setlength\itemsep{0em}
% \item Let \(r \in R\) be any element. If \(m \in \ker p\) then clearly
%   \(r m \in \ker p\) and thus \(q(r m) = r m = r q(m)\).

%   Now, if \(m \in M \setminus \ker p\) one needs caution. If \(r = 0\) then
%   \(r m = 0 \in \ker p\) thus \(q(r m) = 0 = r q(m)\). On the other hand, if
%   \(r \neq 0\) then \(r m \notin \ker p\), thus \(q(r m) = 0 = r q(m)\). This
%   shows that for all \(m \in M \setminus \ker p\) we have
%   \(q(r m) = r q(m) = 0\).

% \item Let \(m, n \in M\) be any two elements. If \(m + n \in \ker p\) then
%   either both \(m, n \in \ker p\), so that \(q(m + n) = m + n = q(m) + q(n)\),
%   or \(m, n \in M \setminus \ker p\), in which case \(p(m + n) = 0\) implies in
%   \(p(m) = -p(n)\), but since \(p^2 = p\), then
% \end{itemize}
\todo[inline]{Prove, I'm stuck in this problem 6.3 from Aluffi}
\end{proof}
