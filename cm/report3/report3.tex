...
\begin{enumerate}[(a)]\setlength\itemsep{0em}
    \item \(F\) is essentially surjective if \(j^* F\) is an essentially
        surjective functor between categories.

    \item The morphism \(F\) is said to be full if, given any sequence \(p_1,
        \dots, p_n, p \in \Obj(\operad P)\), the map
        \[
            F: \operad P(p_1, \dots, p_n; p) \longrightarrow
            \operad Q(F p_1, \dots F p_n; F p)
        \]
        is surjective. On the other hand, \(F\) is said to be faithful if the
        above map is injective. Finally, \(F\) is fully faithful if the above
        map is a bijection.
\end{enumerate}
\end{definition}

\begin{lemma}
A morphism \(F\) of operads is an equivalence if and only if \(F\) is both
...
\todo[inline]{\(\Operad\) is small complete and cocomplete.}
...
\end{itemize}
The proof that the special classes of morphisms are closed under weak
equivalence, and the proof of the 2-out-of-3 property is left as a routine
exercise. We now prove the lifting and factorization properties:
\begin{enumerate}[(a)]\setlength\itemsep{0em}
    \item
...
% FIX: Solve issue, bibliography is not beign printed.
\printbibliography
